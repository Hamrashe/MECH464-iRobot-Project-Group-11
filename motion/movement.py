...
class roomba(object):
    ...
    def odometry():
        ...
        dtheta = math.tan(((left_distance-last_left) - (right_distance-last_right))/(wheel_base)) #Get change in angle
        ...
        dtheta = math.tan(((left_distance-last_left) - (right_distance-last_right))/(wheel_base))
        dtheta_deg = dtheta*180/math.pi
...
if __name__ == "__main__":
    ...
    odom = RepeatTimer(0.01, roomba.odometry)
    odom.start()
    ...
    for vec in dv:
        ...
        print(dist)
        ang = vec[1]-math.pi/2
        rot = angle - ang*180/math.pi
        #print(angle)
        print(rot)
        roomba.rotate(rot, 0.85*np.sign(rot))
